...
fn decode_jpeg_as(
...
) -> Result<(Vec<u8>, usize, usize), KfbError> {
    ...
    let pixels = decoder
        .decode()
        .map_err(|e| KfbError::JpegDecode(format!("{e:?}")))?;
    let info = decoder
        .info()
        .ok_or_else(|| KfbError::JpegDecode("no image info after decode".into()))?;
    Ok((pixels, info.width as usize, info.height as usize))
}

/// Decode a JPEG-compressed byte slice into packed RGB pixels.
///
/// Returns a tuple of `(pixels, width, height)` where `pixels` is a `Vec<u8>`
/// of length `width * height * 3` in row-major, interleaved RGB order.
///
/// # Errors
///
/// Returns [`KfbError::JpegDecode`] if the data is not valid JPEG or if image
/// metadata cannot be retrieved after decoding.
pub(crate) fn decode_jpeg(data: &[u8]) -> Result<(Vec<u8>, usize, usize), KfbError> {
    decode_jpeg_as(data, ColorSpace::RGB)
}

pub(crate) fn decode_jpeg_luma(data: &[u8]) -> Result<(Vec<u8>, usize, usize), KfbError> {
    decode_jpeg_as(data, ColorSpace::Luma)
}
...
mod tests {
    ...
    #[test]
    fn decode_jpeg_returns_rgb_pixels() {
        // Valid 1x1 RGB JPEG (red pixel, generated with Pillow)
        let jpeg: &[u8] = &[
            ...
            0x0C, 0x09, 0x0A, 0x0A, 0x0A, 0xFF, 0xDB, 0x00, 0x43, 0x01, 0x02, 0x02, 0x02, 0x02,
            ...
            0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A,
            0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A,
            0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0xFF, 0xC0, 0x00, 0x11, 0x08, 0x00, 0x01, 0x00, 0x01,
            ...
            0x0B, 0xFF, 0xC4, 0x00, 0xB5, 0x10, 0x00, 0x02, 0x01, 0x03, 0x03, 0x02, 0x04, 0x03,
            0x05, 0x05, 0x04, 0x04, 0x00, 0x00, 0x01, 0x7D, 0x01, 0x02, 0x03, 0x00, 0x04, 0x11,
            0x05, 0x12, 0x21, 0x31, 0x41, 0x06, 0x13, 0x51, 0x61, 0x07, 0x22, 0x71, 0x14, 0x32,
            0x81, 0x91, 0xA1, 0x08, 0x23, 0x42, 0xB1, 0xC1, 0x15, 0x52, 0xD1, 0xF0, 0x24, 0x33,
            0x62, 0x72, 0x82, 0x09, 0x0A, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x25, 0x26, 0x27, 0x28,
            ...
            0xF9, 0xFA, 0xFF, 0xC4, 0x00, 0x1F, 0x01, 0x00, 0x03, 0x01, 0x01, 0x01, 0x01, 0x01,
            0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04,
            ...
            0x00, 0x02, 0x11, 0x03, 0x11, 0x00, 0x3F, 0x00, 0xF8, 0xBE, 0x8A, 0x28, 0xAF, 0xE5,
            0x33, 0xFD, 0xFC, 0x3F, 0xFF, 0xD9,
        ];
    ...
    #[test]
    fn decode_jpeg_luma_returns_single_channel_pixels() {
        // Same 1x1 RGB JPEG as above, decoded to luminance.
        let jpeg: &[u8] = &[
            0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46, 0x49, 0x46, 0x00, 0x01, 0x01, 0x00,
            0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0xFF, 0xDB, 0x00, 0x43, 0x00, 0x02, 0x01, 0x01,
            0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02, 0x02, 0x04, 0x03,
        ...
        let (pixels, width, height) = decode_jpeg_luma(jpeg).unwrap();
        assert_eq!(width, 1);
        assert_eq!(height, 1);
